...
enable_ags() {
  ...
  if command -v ags >/dev/null 2>&1; then
    echo "${INFO:-[INFO]} AGS rilevato - abilitazione negli script di avvio e aggiornamento" 2>&1 | tee -a "$log"
    local OVERLAY_SA="config/hypr/configs/Startup_Apps.conf"
    mkdir -p "$(dirname "$OVERLAY_SA")"
    touch "$OVERLAY_SA"
...
enable_quickshell() {
  ...
  if command -v qs >/dev/null 2>&1; then
    echo "${INFO:-[INFO]} Quickshell rilevato - abilitazione negli script di avvio e aggiornamento" 2>&1 | tee -a "$log"
    local OVERLAY_SA="config/hypr/configs/Startup_Apps.conf"
    mkdir -p "$(dirname "$OVERLAY_SA")"
    touch "$OVERLAY_SA"
...
choose_default_editor() {
  ...
  update_editor() {
    ...
    echo "${OK:-[OK]} Editor predefinito impostato su ${MAGENTA:-}$editor${RESET:-}." 2>&1 | tee -a "$log"
  }
  if command -v nvim &>/dev/null; then
    printf "${INFO:-[INFO]} ${MAGENTA:-}neovim${RESET:-} è rilevato come installato\n"
    if ! read -r -p "${CAT:-[ACTION]} Vuoi impostare ${MAGENTA:-}neovim${RESET:-} come editor predefinito? (s/N): " EDITOR_CHOICE </dev/tty; then
      :
  ...
  if [[ "$editor_set" -eq 0 ]] && command -v vim &>/dev/null; then
    printf "${INFO:-[INFO]} ${MAGENTA:-}vim${RESET:-} è rilevato come installato\n"
    read -r -p "${CAT:-[ACTION]} Vuoi impostare ${MAGENTA:-}vim${RESET:-} come editor predefinito? (s/N): " EDITOR_CHOICE
    if [[ "$EDITOR_CHOICE" == "y" || "$EDITOR_CHOICE" == "Y" || "$EDITOR_CHOICE" == "s" || "$EDITOR_CHOICE" == "S" ]]; then
      update_editor "vim"
      editor_set=1
    fi
  fi
