...
if [[ "$state" == "on" ]]; then
    ...
    echo off > "$STATE_FILE"
    notify-send -u low "Luce notturna: Disattivata" || true
  else
    ...
    echo on > "$STATE_FILE"
    notify-send -u low "Luce notturna: Attivata" "${TARGET_TEMP}K" || true
  fi
}
...
cmd_status() {
  ...
  if [[ "$onoff" == "on" ]]; then
    ...
    cls="on"
    tip="Luce notturna attiva @ ${TARGET_TEMP}K"
  else
    txt="<span size='16pt'>$(icon_off)</span>"
    cls="off"
    tip="Luce notturna disattiva"
  fi
