...
cleanup_backups() {
  ...
  for DIR in "$CONFIG_DIR"/*; do
    ...
    if [ "$mode" = "auto" ]; then
      ...
      echo "${INFO:-[INFO]} Modalità express: backup ridotti per ${YELLOW:-}${DIR##*/}${RESET:-}, mantenuto ${MAGENTA:-}${latest_backup##*/}${RESET:-}." 2>&1 | tee -a "$log"
      continue
    fi

    printf "\n%s Trovati più backup per: %s\n" "${INFO:-[INFO]}" "${DIR##*/}"
    echo "${YELLOW:-}Backup:${RESET:-}"
    for BACKUP in "${BACKUP_DIRS[@]}"; do
      echo "  - ${BACKUP##*/}"
    done
    echo -n "${CAT:-[ACTION]} Eliminare i backup più vecchi e mantenere solo l'ultimo? (s/N): "
    read back_choice
    if [[ "$back_choice" == [YySs]* ]]; then
      for BACKUP in "${BACKUP_DIRS[@]}"; do
        if [ "$BACKUP" != "$latest_backup" ]; then
          rm -rf "$BACKUP"
          echo "Eliminato: ${BACKUP##*/}"
        fi
      done
      echo "Mantenuto: ${latest_backup##*/}"
    fi
  done
}
