...
copy_phase1() {
  ...
  for DIR2 in $dirs; do
    ...
    if [ -d "$DIRPATH" ]; then
      while true; do
        printf "\n${INFO:-[INFO]} Trovata configurazione ${YELLOW:-}$DIR2${RESET:-} in ~/.config/\n"
        echo -n "${CAT:-[ACTION]} Vuoi sostituire la configurazione ${YELLOW:-}$DIR2${RESET:-}? (s/n): "
        read DIR1_CHOICE
        case "$DIR1_CHOICE" in
        [YySs]*)
          BACKUP_DIR=$(get_backup_dirname)
          mv "$DIRPATH" "$DIRPATH-backup-$BACKUP_DIR" 2>&1 | tee -a "$log"
          echo -e "${NOTE:-[NOTE]} - Backup di $DIR2 in $DIRPATH-backup-$BACKUP_DIR." 2>&1 | tee -a "$log"
          cp -r "config/$DIR2" "$HOME/.config/$DIR2" 2>&1 | tee -a "$log"
          echo -e "${OK:-[OK]} - Sostituito $DIR2 con la nuova configurazione." 2>&1 | tee -a "$log"
          if [ "$DIR2" = "rofi" ]; then
          ...
          ;;
        [Nn]*)
          echo -e "${NOTE:-[NOTE]} - Salto ${YELLOW:-}$DIR2${RESET:-}" 2>&1 | tee -a "$log"
          break
          ;;
        *) echo -e "${WARN:-[WARN]} - Scelta non valida. Inserisci S o N." ;;
        esac
      done
    else
      cp -r "config/$DIR2" "$HOME/.config/$DIR2" 2>&1 | tee -a "$log"
      echo -e "${OK:-[OK]} - Copia completata per ${YELLOW:-}$DIR2${RESET:-}" 2>&1 | tee -a "$log"
    fi
  done
}
...
copy_waybar() {
  ...
  if [ -d "$DIRPATHw" ]; then
    while true; do
      echo -n "${CAT:-[ACTION]} Vuoi sostituire la configurazione ${YELLOW:-}$DIRW${RESET:-}? (s/n): "
      read DIR1_CHOICE
      case "$DIR1_CHOICE" in
      [YySs]*)
        BACKUP_DIR=$(get_backup_dirname)
        cp -r "$DIRPATHw" "$DIRPATHw-backup-$BACKUP_DIR" 2>&1 | tee -a "$log"
        echo -e "${NOTE:-[NOTE]} - Backup di $DIRW in $DIRPATHw-backup-$BACKUP_DIR." 2>&1 | tee -a "$log"
        ...
        ;;
      [Nn]*)
        echo -e "${NOTE:-[NOTE]} - Salto la sostituzione della configurazione ${YELLOW:-}$DIRW${RESET:-}." 2>&1 | tee -a "$log"
        break
        ;;
      *) echo -e "${WARN:-[WARN]} - Scelta non valida. Inserisci S o N." ;;
      esac
    ...
    echo -e "${OK:-[OK]} - Copia completata per ${YELLOW:-}$DIRW${RESET:-}" 2>&1 | tee -a "$log"
  fi
}
...
copy_phase2() {
  ...
  for DIR_NAME in $DIR; do
    ...
    if [ -d "$DIRPATH" ]; then
      echo -e "\n${NOTE:-[NOTE]} - Configurazione ${YELLOW:-}$DIR_NAME${RESET:-} trovata, tentativo di backup."
      BACKUP_DIR=$(get_backup_dirname)
      mv "$DIRPATH" "$DIRPATH-backup-$BACKUP_DIR" 2>&1 | tee -a "$log"
    fi
    if [ -d "config/$DIR_NAME" ]; then
      cp -r "config/$DIR_NAME/" "$HOME/.config/$DIR_NAME" 2>&1 | tee -a "$log"
      echo "${OK:-[OK]} - Copia della configurazione ${YELLOW:-}$DIR_NAME${RESET:-} completata!" 2>&1 | tee -a "$log"
      if [ "$DIR_NAME" = "hypr" ] && [ -f "$DIRPATH-backup-$BACKUP_DIR/.biometrics_configured" ]; then
        cp "$DIRPATH-backup-$BACKUP_DIR/.biometrics_configured" "$HOME/.config/hypr/.biometrics_configured"
        echo "${OK:-[OK]} - Flag biometrico ripristinato." 2>&1 | tee -a "$log"
      fi
    else
      echo "${ERROR:-[ERROR]} - La directory config/$DIR_NAME non esiste per la copia." 2>&1 | tee -a "$log"
    fi
  done
...
restore_hypr_assets() {
  ...
  if [ -d "$BACKUP_HYPR_PATH" ]; then
    ...
    echo -e "\n${NOTE:-[NOTE]} Ripristino ${SKY_BLUE:-}Animazioni e Profili Monitor${RESET:-} in ${YELLOW:-}$HYPR_DIR${RESET:-}..."
    ...
    for DIR_RESTORE in "${DIR_B[@]}"; do
      ...
      if [ -d "$BACKUP_SUBDIR" ]; then
        ...
        echo "${OK:-[OK]} - Directory ripristinata: ${MAGENTA:-}$DIR_RESTORE${RESET:-}" 2>&1 | tee -a "$log"
      fi
    done
    ...
    for FILE_RESTORE in "${FILE_B[@]}"; do
      ...
      if [ -f "$BACKUP_FILE" ]; then
        ...
        echo "${OK:-[OK]} - File ripristinato: ${MAGENTA:-}$FILE_RESTORE${RESET:-}" 2>&1 | tee -a "$log"
      fi
    done
  fi
}
...
restore_user_configs() {
  ...
  if [ -d "$BACKUP_DIR_PATH" ] && [ "$SKIP_RESTORE_PROMPTS" -eq 0 ]; then
    ...
    echo -e "${NOTE:-[NOTE]} Ripristino ${MAGENTA:-}User-Configs${RESET:-} precedenti... " 2>&1 | tee -a "$log"
    printf "${WARNING:-}\\
    █▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀█\\n\\
            NOTE per il RIPRISTINO DELLE CONFIGURAZIONI PRECEDENTI\\n\\
    █▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄█\\n\\n\\
    La cartella 'UserConfigs' contiene tutte le tue impostazioni personali.\\n\\
    I file in questa cartella sovrascrivono le configurazioni predefinite,\\n\\
    così le tue personalizzazioni non vengono perse durante gli aggiornamenti.\\n\\
" >&2

    if version_gte "$CURRENT_VERSION" "$TARGET_VERSION"; then
      read -r -p "${CAT:-[ACTION]} Vuoi ripristinare la tua cartella UserConfigs precedente? (s/N): " restore_userconfigs_dir
      if [[ "$restore_userconfigs_dir" != [Nn]* ]]; then
        echo "${NOTE:-[NOTE]} Ripristino cartella UserConfigs in corso..." 2>&1 | tee -a "$log"
        rsync -a "$BACKUP_DIR_PATH/" "$DIRPATH/UserConfigs/" 2>&1 | tee -a "$log"
        echo "${OK:-[OK]} - Cartella UserConfigs ripristinata." 2>&1 | tee -a "$log"
      else
        echo "${NOTE:-[NOTE]} - Ripristino UserConfigs saltato." 2>&1 | tee -a "$log"
      fi
    else
